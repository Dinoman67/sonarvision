"""
================================================================
YOLOv8-ESI Training — SonarVision Multi-Source Dataset v3 (FINAL)
================================================================
5-class detection: unknown_debris, airplane, drowning_victim, mine, wreck
3 sensors: NOAA e4+h8 | MILCO-NOMBO | Kaggle SSS   (5570 images, leakage-free)
...
GOAL (SIH):
  ...
  This run trains the SAME YOLOv8-ESI architecture on the FINAL
  MULTI-CLASS, MULTI-SENSOR v3 dataset so the model:
...
v3 = SENSOR-ISOLATED classes (each class has ONE visual identity):
  - unknown_debris = NOAA debris + MILCO NOMBO debris (catch-all class;
    2 real sensors help cross-sensor debris, NOMBO = mine's in-image
    negatives)
  - mine = MILCO ONLY (Kaggle mines dropped — different domain: crop
    corr 0.26, opposite aspect ratio, 19x size; caused a bimodal class)
  - airplane / drowning_victim / wreck = Kaggle
  Audited clean: 0 cross-split leakage, uniform 512x512, 0 junk boxes
  (see audit_dataset_definitive.py + reports/*_v3*.md)
...
CHANGES vs earlier training:
  - v3 dataset: richer real-world data + MILCO mines now a clean
    single-sensor class (435 boxes)
...
print('Upload sonarvision_multisource_v3.zip:')
uploaded = files.upload()
zip_path = list(uploaded.keys())[0]
with zipfile.ZipFile(zip_path, 'r') as z:
    z.extractall('/content/')

DATASET_DIR = '/content/datasets/sonarvision_multisource_v3'
...
VARIANTS = {
    'yolov8n_esi': {'base': 'yolov8n.pt', 'name': 'model_esi_v3_n_s1'},
    'yolov8s_esi': {'base': 'yolov8s.pt', 'name': 'model_esi_v3_s_s1'},
}
