...
v2 → v3 changes (sensor-isolated classes — FINAL design):
  1. mine = MILCO ONLY. Kaggle mines (visually incompatible domain, ~0.26
     crop correlation, opposite aspect, 19x size difference) are dropped so
     every class has ONE visual identity and no cross-sensor conflict.
  2. unknown_debris = NOAA debris + MILCO NOMBO debris. Debris is a
     catch-all class (no single visual identity), so 2 real sensors help
     cross-sensor debris generalization. NOMBO boxes also act as the
     in-image negatives that make MILCO mine detection learnable.
  3. airplane / drowning_victim / wreck = Kaggle (single sensor each).
  4. Kept from v2: NOAA sequence-aware grouping, augmented copies inherit
     source group key (leakage-free), junk-box filter, 512x512 normalize,
     deterministic seed.

Master classes:
  0: unknown_debris   (NOAA marine_debris + MILCO NOMBO)
  1: airplane         (Kaggle)
...
Usage:
    python scripts/build_multisource_dataset_v3.py \
        --output datasets/sonarvision_multisource_v3 \
        --seed 42
"""
...
MILCO_MAPPING = {0: 3, 1: 0}  # MILCO -> mine, NOMBO -> unknown_debris
...
def group_aware_split(samples, seed=42):
    ...
    total_parts = max(1, round(n_total / 50))
...
def generate_report(samples, output_dir, dup_count):
    ...
    # JSON
    json_path = os.path.join(report_dir, "dataset_audit_multisource_v3.json")
...
def main():
    ...
    # Step 6: Split
    logger.info("=" * 60)
    logger.info("STEP 6: Group-aware splitting")
    logger.info("=" * 60)
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("DATASET V3 BUILD SUMMARY")
    logger.info("=" * 60)
