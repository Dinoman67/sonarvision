"""
================================================================
YOLOv8-ESI v6 — 4 classes, h8-only debris, Core_model recipe
================================================================
...
Two fixes vs the notebook:
  1. The notebook's Stage 2 RE-BUILT the model from scratch (its patched
     trainer ignored the Stage-1 weights). v6 Stage 2 properly loads
     Stage-1 best.pt onto the ESI architecture (0 missing / 0 unexpected
     keys asserted) — i.e. true "50 more epochs".
  2. cls_pw=1.0 restored (standard full positive weight to prevent low
     confidence suppression on small objects like mine and debris).

DATASET (v6, audited):
  - 4 classes: unknown_debris (h8 Core_model split) / airplane (Kaggle) /
    mine (MILCO 5-point fix) / wreck (Kaggle).
  - Debris: Restored proven frame-level split (h8/train -> train,
    h8/val -> val, h8_unseen_test -> test). Zero file overlap.
  - Mine: NOMBO mapped to mine (resolves 74 contradictory frames), background
    capped to 1:0.5 ratio, corrupt sub-pixel boxes removed, stratified per-year.
...
# ── CONFIG — edit this to where you uploaded the v6 zip ──────────
DRIVE = '/content/drive/MyDrive'          # zip is directly at Drive root
ZIP   = f'{DRIVE}/sonarvision_multisource_v6.zip'
...
# v4-style zip nests under datasets/sonarvision_multisource_v6/...
# v6-current zip is FLAT (dataset.yaml, images/ at the zip root).
found = None
...
OUT = '/content/results'
OUT_DRIVE = f'{DRIVE}/results_v6'        # persistent copy on Drive
...
# Set CLS_PW to 1.0 (standard full weight for positive classes to prevent low confidence suppression)
CLS_PW = 1.0
...
print(f'v6 config (Core_model recipe) | imgsz={IMGSZ} '
...
try:
    ...
    model.train(data=DATA, imgsz=IMGSZ, batch=BATCH1, cls_pw=CLS_PW,
                **STAGE1, **TRAIN_CONFIG, name='model_esi_v6_s1')
finally:
    DetectionTrainer.get_model = _orig

BEST1 = f'{OUT}/model_esi_v6_s1/weights/best.pt'
...
for _f in ['best.pt', 'last.pt']:
    _p = f'{OUT}/model_esi_v6_s1/weights/{_f}'
...
try:
    ...
    model2.train(data=DATA, imgsz=IMGSZ, cls_pw=CLS_PW,
                 **STAGE2, **TRAIN_CONFIG, name='model_esi_v6_s2')
finally:
    DetectionTrainer.get_model = _orig2

BEST2 = f'{OUT}/model_esi_v6_s2/weights/best.pt'
...
for _f in ['best.pt', 'last.pt']:
    _p = f'{OUT}/model_esi_v6_s2/weights/{_f}'
    if os.path.exists(_p):
...
# ================================================================
import csv
src_root = Path('/content/source_eval_v6')
...
print('=' * 60)
EXPORT_DIR = Path('/content/yolo_esi_v6_exports')
...
for fmt in ['fp32', 'fp16']:
    ...
    try:
        ...
        dest = EXPORT_DIR / f'yolo_esi_v6_{fmt}.onnx'
...
pd.DataFrame(summary).to_csv('/content/v6_eval_summary.csv', index=False)
_saved = []
for src_file in [BEST, BEST1, BEST2, f'{OUT}/model_esi_v6_s1/weights/last.pt',
                 f'{OUT}/model_esi_v6_s2/weights/last.pt',
                 '/content/v6_eval_summary.csv']:
    if os.path.exists(src_file):
        shutil.copy2(src_file, f'{OUT_DRIVE}/{Path(src_file).name}')
        _saved.append(Path(src_file).name)
_fp16 = EXPORT_DIR / 'yolo_esi_v6_fp16.onnx'
if _fp16.exists():
    shutil.copy2(_fp16, OUT_DRIVE)
    _saved.append(_fp16.name)
shutil.make_archive('/content/yolo_esi_v6', 'zip', str(EXPORT_DIR))
...
print('  v6 features restored: debris frame-holdout + MILCO 5-point fix.')