...
v1 → v2 improvements:
  1. Added NOAA h8 dataset (+3548 images, 633 debris boxes + 2915 background)
  2. Oversampled minority classes: drowning_victim 10x, airplane 3x
  3. Sonar noise pipeline: Gaussian, speckle, brightness jitter on augmented images
  4. Better class balance for 5-class detection

Master classes:
  0: unknown_debris   (NOAA marine_debris + MILCO NOMBO)
  1: airplane         (Kaggle)
  2: drowning_victim  (Kaggle — oversampled)
  3: mine             (Kaggle mine + MILCO MILCO)
  4: wreck            (Kaggle wreck/shipwreck)

Usage:
    python scripts/build_multisource_dataset_v2.py \
        --output datasets/sonarvision_multisource_v2 \
        --seed 42
"""
...
KAGGLE_MAPPING = {0: 1, 1: 2, 2: 3, 3: 4}  # airplane, drowning, mine, wreck
...
def generate_report(samples, output_dir, dup_count):
    ...
    # JSON
    json_path = os.path.join(report_dir, "dataset_audit_multisource_v2.json")
    ...
    with open(md_path, "w") as f:
        ...
        f.write("- Added sonar noise pipeline (Gaussian, speckle, brightness)\n\n")
        f.write("## Overall\n\n")
...
def main():
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("DATASET V2 BUILD SUMMARY")
    logger.info("=" * 60)
